...
impl HppParser {
    pub fn new(content: &str) -> Result<Self, Codes> {
        ...
        let temp_file = NamedTempFile::new().map_err(|_e| vec![])?;
        fs::write(temp_file.path(), content).map_err(|_e| vec![])?;
        ...
        let path = workspace.join(temp_file.path().file_name().unwrap().to_str().unwrap()).map_err(|_e| vec![])?;

...
impl ClassHierarchyWriter {
    ...
    fn write_metadata(
        &self,
        classes: &[arma3_database::models::class::ClassModel],
    ) -> ReporterResult<()> {
        let file_path = self.output_dir.join("class_metadata.csv");
        let file = File::create(&file_path).map_err(|e| ReporterError::WriteError {
        ...
        // Write header
        writeln!(writer, "id;color;type").map_err(|e| ReporterError::WriteError {
            path: file_path.clone(),
            source: e,
        })?;
        ...
        for class in classes {
            ...
            writeln!(writer, "{};{};{}", class.id, color, group_type).map_err(|e| {
        ...
        writer.flush().map_err(|e| ReporterError::WriteError {
            path: file_path.clone(),
            source: e,
        })?;
    ...
    pub fn write_class_graph(&self, class_repo: &ClassRepository) -> ReporterResult<()> {
        ...
        std::fs::create_dir_all(&self.output_dir).map_err(|e| ReporterError::CreateDirError {
        ...
        // Write header
        writeln!(writer, "source;target").map_err(|e| ReporterError::WriteError {
            path: file_path.clone(),
            source: e,
        })?;
        ...
        for (parent, children) in &child_map {
            for child in children {
                writeln!(writer, "{};{}", parent, child).map_err(|e| {
                    ReporterError::WriteError {
                        path: file_path.clone(),
                        source: e,
                    }
        ...
        info!(
            "Class hierarchy graph written to: {} with {} edges",
            file_path.display(),
            count
        );
        Ok(())
...
mod tests {
    use super::*;
    use arma3_database::{models::class::ClassModel, DatabaseManager};

...
fn create_indexes(tx: &rusqlite::Transaction) -> Result<()> {
    ...
    // Class property indexes
    tx.execute(
        "CREATE INDEX IF NOT EXISTS idx_class_properties_class ON class_properties(class_id)",
        [],
    )?;
    tx.execute(
        "CREATE INDEX IF NOT EXISTS idx_class_properties_name ON class_properties(property_name)",
        [],
    )?;
